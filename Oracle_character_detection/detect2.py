...
#     w, h = img.size
#     resize_w = w if w % 32 == 0 else int(w / 32) * 32
#     resize_h = h if h % 32 == 0 else int(h / 32) * 32
...
def process_directory(input_dir, output_img_dir, output_boxes_dir, model, device):
    ...
    for img_file in img_files:
        ...
        if boxes is not None:
            output_boxes_path = os.path.join(output_boxes_dir, img_file.replace('.png', '.txt').replace('.jpg', '.txt').replace('.jpeg', '.txt'))
...
if __name__ == '__main__':
    input_dir = '/home/dengbinquan/XIAOWU/EAST/test_data'  # 输入图片的文件夹路径
    output_img_dir = '/home/dengbinquan/XIAOWU/EAST/test_data_result'  # 保存带检测框图像的文件夹路径
    output_boxes_dir = '/home/dengbinquan/XIAOWU/EAST/test_data_tag'  # 保存检测框信息的文件夹路径
    model_path = '/home/dengbinquan/XIAOWU/EAST/pths/model_epoch_600.pth'  # 模型权重文件路径
