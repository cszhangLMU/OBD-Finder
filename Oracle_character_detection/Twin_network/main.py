...
os.environ['CUDA_VISIBLE_DEVICES'] = '1'  # Use GPU device 1
        ...
        for j in range(i, len(dataset)):
            ...
            for crop1, box1 in zip(img1_crops, boxes1):
                for crop2, box2 in zip(img2_crops, boxes2):
                    ...
                    if i == j:
                        labels.append((1, 1, 1))  # Positive sample
                    else:
                        labels.append((0, 0, 0))  # Negative sample
...
class OraclePairDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img1_size = img1.size(1), img1.size(2)  # Get (width, height)
        box1 = self.convert_bbox(box1, img1_size)
        box2 = self.convert_bbox(box2, img1_size)

        return img1, img2, torch.tensor(box1, dtype=torch.float32), torch.tensor(box2,
...
class AdvancedSiameseNetwork(nn.Module):
    def __init__(self):
        ...
        # Compute flattened feature vector size
        self._initialize_fc_layers()

    def _initialize_fc_layers(self):
        # Used to calculate flattened feature size
        with torch.no_grad():
            dummy_input = torch.zeros(1, 3, 224, 224)  # Dummy input tensor
            dummy_output = self.feature_extractor(dummy_input)
            dummy_output = self.se_block(dummy_output)
            dummy_output = self.additional_conv(dummy_output)
            flattened_size = dummy_output.view(-1).size(0)  # Flattened size

        # Use flattened_size for fully connected layer input
...
# Advanced training and evaluation strategy
...
learning_rate = 0.001
num_epochs_per_batch = 10  # Number of epochs per batch
batch_size = 8
step_size = 10
gamma = 0.1
...
# Get all XML annotation file paths
...
# Split file list into 5 groups, 100 files each
num_batches = 5
batch_size_files = len(annotation_files) // num_batches
annotation_batches = [annotation_files[i:i + batch_size_files] for i in
                      range(0, len(annotation_files), batch_size_files)]
